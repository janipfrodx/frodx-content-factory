...
Pot `scripts/init_run.py` je relativna na mapo tega skilla (`plugins/content-factory/skills/frodx-content-factory/`); `runs` (in kasneje `outbox/`) nastane relativno na CWD ob zagonu ukaza. Cowork-ovo dejansko CWD ob zagonu skilla ni preverjeno v tej seji - Jani to preveri ob prvi živi namestitvi (Task 12) in po potrebi popravi na absolutno pot.
